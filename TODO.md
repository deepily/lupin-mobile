...
- [ ] [LUPIN-MOBILE] 🔴 **NEXT ACTION, ONE SITTING, CLOSES TWO ROWS** — bounce `:7999` (`lupin/src/scripts/bounce-dev-server.sh`; auto-reload is OFF, verified on the live container, so the merged fix is **not** the served code), then Rick asks a calculator question **twice** and expects the answer **both** times. That single run is the receipt for `0e7c9214` **and** the clean AC-G3 read that closes `734bd1bf`. ⚠️ Running it *before* the bounce measures the old bug and reads as a failed fix.
