...
- [~] **Phase 5 `384591dd` — BUILT 2026-09-22 (Tiffany, on Rick's order), `c990659` + `13ad75f`.** Acceptance implemented as the settled *guard absence* shape: `AckConfidence.interrupted` renders "acks could not be confirmed" and **never a denominator**, and `fold()` cannot restore confidence. Suite **1645 / 1 skipped / 0 failed**, up from 1586. 59 new tests; mutations 3/3 RED on the data layer and 4/4 RED on the pane.
  - 🔴 **The plan's §6.2 drain is implemented as a REFUSAL that throws.** It cannot return an ack, so written as specified it would have run, recovered zero, and stayed green.
  - ⚠️ **NOT DONE: wiring.** The pane has no home-screen card, no provider, and `app.dart` does not route `commons_broadcast_ack` to it — so the tally has no source of acks in the running app. Gated on Rick's answer about where the bloc should live, because `app.dart` can only dispatch to a bloc it can see.
...
- [ ] 🔴 **`props`-AS-SUMMARY — the defect I fixed in Broadcast (`9702fe3`) EXISTS ELSEWHERE, and I checked rather than assumed.** bloc skips an emit when the new state compares equal, so a `props` entry that is a **length or a count standing in for a value** silently drops rebuilds: no exception, no red test, just a screen that stops updating. Audited every `get props` in `lib/`:
  ...
  - **Not fixed by me** — outside Phase 5 and, for Decision Proxy, outside the fleet panes entirely. Raised for a scope call, same reasoning as the orphan panes.
- [ ] 🔴 **ORPHAN PANES — four of five fleet surfaces are unreachable in the running app.** Measured 2026-09-22 by enumeration: `TaskListPane`, `HoldingAreaPane`, `FinishedTasksScreen` and `BroadcastPane` are each referenced ONLY by their own file and their tests — no home-screen card, no route, no `BlocProvider` in `app.dart`. The home screen has six cards: Job Queue, Claude Code, Notifications, Trust Dashboard, Agentic Jobs, Fleet Status. Only `FleetStatusScreen` is reachable (`home_screen.dart:148`). Four phases of tested, merged, green work that nobody can open on a phone.
  ...
  - Raised to Rick; scope call pending, since fixing it touches three other phases' code.
