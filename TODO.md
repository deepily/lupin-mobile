...
### Decisions Log addendum — 2026-09-22 ritual

- 🔴 **THE MANIFEST IS WRITTEN LAST, SO IT MUST BE BACKED UP LAST.** Ritual order put the backup before recording its own receipts, so `.claude-session.md` reached the mirror **9 minutes stale** and without the receipt block — measured, not assumed: mirror `22:54` vs source `23:03`, and a grep for the receipts returned 0. Rick's follow-up question ("have you pushed and backed up also?") is what surfaced it; a second `--write` run fixed it in one file. ⇒ **A ritual that backs up before writing its own receipts always leaves the mirror one edit behind.** Either back up after the manifest write, or run it twice.
- ✅ **Receipts, 2026-09-22 23:0x**: commit `75f1291` (15 files) · push `9d6e396..75f1291`, **16 commits**, origin in sync · backup `--write` exit 0, **verified by reading files back off the mirror** rather than trusting "SYNC COMPLETE" · suite **1673 passed / 1 skipped / 0 failed**.
- **Commit messages go to a FILE and use `git commit -F`.** Verified 7 identifiers survived; a `-m` string ate four of them earlier the same day.
