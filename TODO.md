...
- [ ] 🔴 **ORPHAN PANES — four of five fleet panes are unreachable in the running app.** Measured 2026-09-22: `TaskListPane`, `HoldingAreaPane`, `FinishedTasksPane` and `BroadcastPane` are each referenced ONLY by their own file and their tests — no home-screen card, no route, no `BlocProvider` in `app.dart`. Only `FleetStatusPane` is reachable (`home_screen.dart:148`). Four phases of tested work that nobody can open. Raised to Rick; scope call pending, since it touches three other phases' code.
