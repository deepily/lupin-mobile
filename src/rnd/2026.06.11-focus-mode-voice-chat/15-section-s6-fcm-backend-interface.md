...
- `POST /api/fcm/register-token` (JWT) — body `{ "token": str, "platform": "android",
  ...
  user allowed.
- `DELETE /api/fcm/register-token` (JWT) — body `{ "token": str }` → 200. Best-effort logout path.
  *Parent-design flag (Stage-1 residual)*: DELETE-with-JSON-body is legal in FastAPI/Dio but
  proxy-fragile — parent design confirms or switches to `POST /api/fcm/unregister-token` under
  the OSQ-6 amendment rights below.
...
- Data-only FCM HTTP v1 message, `android.priority: HIGH` (Doze delivery), payload
  ...
  NO sender identity beyond what reconnect will fetch anyway.
...
- [ ] Firebase service-account credential wiring (env-var path, never committed; INI-named).
...
- [ ] WS client-type marker: record `client_type` from `auth_request`; absent ⇒ web (F-S6-1).
...
- [ ] Token registry: storage + register/unregister endpoints per §3.1. **Durability
      ...
      OSQ-6.
- [ ] Wake trigger + debounce per §3.3, hooked at the notification fan-out point where
      ...
      parent-defined).
- [ ] INI keys (`fcm *` family) + splainer entries
- [ ] Parent-side unit tests + endpoint tests (parent venue rules apply: `:7999`-eligible if
      net-zero, else `:8000` scheduled)
- [ ] Documentation touchpoints per parent CLAUDE.md (rest-api-reference, notification docs,
      websocket-events/architecture for the client-type marker)

## 5. Acceptance Criteria

- [ ] EXECUTOR: AI (parent session; executability conditional on OSQ-6 resolution — the storage
...
- [ ] EXECUTOR: AI (parent session) — AC-S6.2: wake fires for token-registered user with no live
...
- [ ] EXECUTOR: AI (parent session) — AC-S6.3: outgoing FCM payload asserts data-only +
...
- [ ] EXECUTOR: AI (cascade instant: Stage-3 reviewer, at S5's Stage-3 close
...
- [ ] Parent-side green baseline (relevant suite scope) recorded BEFORE first edit: `____`
      (date/time, command, count)
