...
**Status**: PLAN **rev 12** (see §7; the revision number lives there and here only — and rev 10 broke exactly that rule,
...
⚠️ **One item is PROVISIONAL and must not be read as settled: CB4** — the `FormData` factory is **Mr. Radio's call, not
Rick's**. The ask to Rick timed out after the full 600 seconds and **a timeout is not a ruling**. Build on it meanwhile;
if he rules the other way, that one item backs out alone.
...
| # | Guarantee |
|---|---|
| D1 | A non-200 status means **nothing was asked** (401, 422 empty speech, 503 GPU OOM or flow disabled, **500 any other transcribe failure**). ⚠️ **The 500 is part of this enumeration, not an afterthought (D3)**: rev 9 added the generic `except Exception` to §2.2 and updated nothing else, leaving the branch alive in the pseudocode and invisible in all three places that enumerate this door's error behaviour. The guarantee itself still holds — a 500 means nothing was asked — so this was an incomplete *enumeration*, never a broken promise |
...
| Any other transcribe failure | **500** with the named detail, body shape matching `speech.py:906-913`; `flow.ask` never called and the temp file still removed. **D3** — rev 9 added this branch to §2.2 and updated nothing else, so it lived in the pseudocode and in none of the three places that enumerate this door's behaviour |
...
- **Send immediately**:
  ...
  8. 🔴 **AND releases the recording: `_asr.discardPendingUpload()` (D2).** §3.2 rules that `AsrService` retains
     ...
     event (Result, Failed, CutOff) **and** on the path where `askSpoken` is never subscribed at all — a lazy `async*`
...
`quick_ask_event.dart` (CC6's public event) · `quick_ask_bloc.dart` (the branch, `_spokenSubs`, the `@visibleForTesting` accessor at step 7, the disposal call at step 8) ·
