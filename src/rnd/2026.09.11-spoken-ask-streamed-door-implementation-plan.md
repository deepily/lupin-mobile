...
**Status**: PLAN **rev 14** (see §7; the revision number lives there and here only — and rev 10 broke exactly that rule,
...
| # | Guarantee |
|---|---|
| D1 | A non-200 status means **nothing was asked** (401, 422 empty speech, 503 GPU OOM or flow disabled, **500 any other failure reading, saving or transcribing the audio** — E4b widened this from "transcribe" at rev 13, see §2.2). ⚠️ **The 500 is part of this enumeration, not an afterthought (D3)**: rev 9 added the generic `except Exception` to §2.2 and updated nothing else, leaving the branch alive in the pseudocode and invisible in all three places that enumerate this door's error behaviour. The guarantee itself still holds — a 500 means nothing was asked — so this was an incomplete *enumeration*, never a broken promise |
...
async def ask_audio( file: UploadFile = File( ... ), websocket_id: Optional[ str ] = None, speak: bool = True,
    ...
    try:
        ...
        # correction's other half, is deliberately absent: read, save and transcribe are all one thing from
        # the client's side (the server could not accept the audio), so the client gets one message.
...
| Any other failure reading, saving or transcribing the audio | **500** with the fixed detail `"Could not accept the audio. Nothing was asked."` and no exception text; `flow.ask` never called and the temp file still removed (or never created). 🔴 **E4 — rev 12 said "body shape matching `speech.py:906-913`", and both halves of that citation were wrong.** The **extent**: those lines are the WAV door's *handlers*, of a `try` that opens at **:864**. The **start digit**: the first `except` is **:907**, not 906. And there was no single shape to match — WAV `:913` sends `str( e )`, MP3 `:519-528` a fixed message — so §2.2 now follows the MP3 correction and says why. **E4b** — a read or save failure (`OSError` from `save_audio_upload`) is in this row too, because §2.2 moved both statements inside the `try`; at rev 12 it escaped as a bare FastAPI 500. **D3** — rev 9 added this branch to §2.2 and updated nothing else, so it lived in the pseudocode and in none of the three places that enumerate this door's behaviour |
...
| **SB-fix — the fixture is copied WHOLE, and its provenance rides INSIDE it.** One file, no sidecar: `ask_audio_ndjson_contract.json` is the lupin file byte for byte, `_capture` block included, so the recorded lupin sha travels with the body it describes. **A sidecar is the wrong shape here** — the tree uses one for a *media* file (`canned-focus-mode-test.provenance.json`, whose `"fixture"` key names the WAV) and an *inline* block for a *contract* (`transcribe_endpoint_contract.json`, `asr_contract_test.dart:39`). Two files can drift apart; a file cannot drift from its own header. ⚠️ **Recording the sha is not the guard — asserting it is** (see §3.4) | `test/fixtures/asr/ask_audio_ndjson_contract.json` (no `.provenance.json`) |
...
- **Send immediately**:
  1. Capture the epoch (as :194), set phase `transcribing`, `stopToFile()`, then subscribe and **store the handle in a
     ...
     **and** the `askSpoken(...).listen(...)` in a **`try`/`catch`**. On a throw: call `_asr.discardPendingUpload()`
     (a no-op when `stopToFile()` threw before any path was retained), emit idle with the error as step 4 does, and
     **do not** add a key to `_spokenSubs`. Rev 12's step 8 demanded disposal "on the path where `askSpoken` is never
  ...
  8. 🔴 **AND releases the recording: `_asr.discardPendingUpload()` (D2).** §3.2 rules that `AsrService` retains
...
| B | Unit | 🔴 **Fixture CURRENCY, not presence (SB-fix).** The phone's `ask_audio_ndjson_contract.json` is asserted **byte-identical to lupin's**, and its `_capture.lupin_sha` is asserted to equal the sha the lupin fixture was committed at. A stale copy fails the run. ⚠️ **The existing precedent is the weak version and must not be copied**: `asr_contract_test.dart:39` asserts only `contract['_capture'] != null` — *"provenance block present"* — which passes forever no matter how far the copy drifts. ⚠️ **Open, named rather than invented (Sam's)**: the sha-equality assertion needs a source for lupin's sha — a committed value, a CI-passed variable, or a checked-in copy of the lupin fixture. That is the §B builder's implementation choice | `test/unit/queue/ask_spoken_stream_test.dart` |
...
**Files this plan touches, §B/§C side, stated once so nothing hides**: `asr_service.dart` (SB1 extract, `_pendingUploadPath`,
...
2. §3.4 asserts the copy is **byte-identical to lupin's** and that `_capture.lupin_sha` equals the sha the lupin fixture
   was committed at — **currency, not presence**.
