...
**Status**: PLAN **rev 7** (see §7; the revision number lives there and here only). Rick approved it "for the most part"
and asked for a cascaded review before building. **Step 0 is CLEAR** — Chloé's thumbs-up at rev 5, her three nits folded
as rev 6 (N2) and rev 7 (N1, N3); no code has changed since rev 5. Next: the Manager mints the section tier and Sam
opens Stage 1 on §A. **Nothing is built until the cascade closes and Rick okays the revised plan.**
...
async def ask_audio( file: UploadFile = File( ... ), websocket_id: Optional[ str ] = None, speak: bool = True,
                     ...
                     active_tasks: dict = Depends( speech.get_active_tasks ) ):
    user_id, user_email = _identity_or_401( current_user )         # NEW helper: the checks /api/v2/ask inlines at :408-415
    ...
    # insurance plus a cancellation handle, not as a proven fix. House shape: speech.py:643 and :797
    # register in the active_tasks dict behind get_active_tasks (speech.py:249); peer.py:409 likewise.
    task_key = f"ask-audio:{session_id}:{uuid.uuid4().hex[ :8 ]}"   # NOT bare session_id: two asks in
    active_tasks[ task_key ] = ask_task                             # one session would evict the first

    # One callback, two jobs: drop the registration, and retrieve the exception so a client that
...
`run_in_threadpool`, pydantic, `ConfigurationManager`, `get_current_user` — verified at `5ff67833`): `asyncio`, `uuid`,
`File` and `UploadFile` from `fastapi`, `Request` if the trace block needs it, `StreamingResponse` from
`fastapi.responses`, `InputAndOutputTable`, and `cosa.rest.routers.speech` for the reused helpers and seams.

**NEW functions in `v2_ask.py`**: `_identity_or_401`, `_insert_io_row`, `_ndjson`, `_stream_headers`. **Reused**: `speech.audio_suffix_from_filename`
...
|---|---|---|
| Unit | Body is exactly two lines in order; `FakeFlow` records that line 1 was already yielded when `ask` ran; the body equals the committed contract fixture | `src/tests/unit/test_v2_ask_audio.py` (model `test_v2_ask.py:39-64`; `dependency_overrides` for `get_current_user`, `v2_ask.get_ask_flow`, `speech.get_speech_provider`, `speech.get_config_manager`, **`speech.get_active_tasks`** — the seam does `import lupin_app.main`, so a bare test app raises without it, and arm (e) asserts on that dict) |
