...
**Status**: PLAN **rev 11** (see §7; the revision number lives there and here only — and rev 10 broke exactly that rule,
...
- **Send immediately**:
  ...
  7. 🔴 **AND releases the recording: `_asr.discardPendingUpload()` (D2).** §3.2 rules that `AsrService` retains
...
| B | Unit | 🔴 **CB4 — 401-then-retry actually replays.** Fake adapter returns 401 once, then 200 with the two NDJSON lines; assert the retry carries a **fresh, unconsumed** multipart body and the stream still yields Transcript then Result. Without this row the factory fix ships unguarded, and the failure it prevents is invisible until a token expires mid-question | `test/unit/queue/ask_spoken_stream_test.dart` |
