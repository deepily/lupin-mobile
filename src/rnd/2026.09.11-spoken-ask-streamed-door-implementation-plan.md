...
**Status**: PLAN **rev 13** (see §7; the revision number lives there and here only — and rev 10 broke exactly that rule,
...
total 99%, because `v2_ask.py` misses **7 statements at `:246-253`** — the `_scopes()` closure inside `build_ask_flow`,
which **no unit test reaches**. That code is pre-existing. A2's gate is unconditional 100% on the post-fold files, so
the §A builder owes a test for that closure (or a `# pragma: no cover` with a same-line reason) **in addition to** covering
the new door. Green at Form 2 is §4 step 1's **exit** criterion, not its starting state.
