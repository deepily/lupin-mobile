...
**Status**: Plan — awaiting Rick's design-confirm gate before any code change.
**Anchor**: mobile commit `fd8fc18` (2026-05-06 Phase 0 dispatch audit)
**Source inventory**: locked notification-UI change audit delivered to Rick this morning + DM exchange with Mr. Radio 🦉 (session `679e8f04`)
...
**Out of scope for this sync** (deferred to separate plans if/when prioritized):
- Recent-Activity filter strip UI (Mr. Radio's Part A) — web-only UX, no mobile surface
- Focus-bar chronological lock UI (Mr. Radio's Part B) — web-only UX, no mobile strip
- Commons DM panel / Recent Activity stream — CC-session features, mobile is not a CC peer
- Doc viewer link emission — mobile does not render `/app/docs` links anywhere today (verified via grep — zero refs)
- `preferred_persona_name` allocate query param — mobile does not allocate personas
- TTS preview-and-pause `tts_preview_*` config fields — mobile does not consume `/api/config/client`
...
5. Update `TODO.md` to close the "May 6 → May 21 notif-client sync" line item; add the deferred items (filter strip, chronological lock, DM panel, Recent Activity stream) as separate parking-lot entries if Rick wants them tracked
...
| `TODO.md` | 5 | Close sync line item + park deferred items |
...
| Test count grows but coverage on `notification_bloc.dart` drops below project floor | low | Track c8/lcov per file in Phase 5; the new code paths are well-isolated and each adds at least one direct test |
...
Plan-of-record. Implementation does NOT begin until Rick greenlights this proposal. Mr. Radio is aware via the morning DM exchange that mobile parity work is in planning; he'll coordinate Phase 4 backend-plumbing only if his Part B uncovers a gap.
