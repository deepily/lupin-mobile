...
**Status**: ✅ COMPLETE 2026-05-07 (uncommitted)

### Tasks

- [x] EXECUTOR: AI — Create `lib/features/notifications/presentation/persona_badge.dart` as a `StatelessWidget` **wrapping `CircleAvatar`** (per REUSE pre-pass `extend-existing` finding — see `00-index.md` "Prior art referenced"; sibling pattern at `inbox_screen.dart:141`). Emoji from `persona.icon` rendered as the avatar child; background color from `persona.color`; long-press shows `displayName` tooltip; dashed-border variant when `borrowed=true` via a `CustomPaint` overlay using the new `DashedBorderPainter` (per `Q2`). **Executed 2026-05-07**: hex-color parser with theme-primary fallback; foreground (emoji color) chosen by `Color.computeLuminance() > 0.5` so emoji+border contrast against dark backgrounds; `Tooltip(triggerMode: longPress)` per spec.
- [x] EXECUTOR: AI — Create `lib/shared/painters/dashed_border_painter.dart` — a `CustomPainter` subclass drawing a dashed stroke around its boundary `Rect`. Per `Q9` (FROZEN at REUSE pre-pass — confirmed genuinely-new; no existing `CustomPainter` subclass and no dashed-border package in `pubspec.yaml`). **Executed 2026-05-07**: 60-line painter using `Canvas.drawArc` in a stepped loop; configurable `strokeWidth`/`dashLength`/`gapLength`; `shouldRepaint` checks all params.
- [x] EXECUTOR: AI — Add `TestKeys` constants to `lib/core/testing/test_keys.dart` for badge + dashed variant. **Executed 2026-05-07**: added `personaBadgePrefix` + `personaBadgeDashedPrefix` (suffixed with `senderId` at use-site).
- [x] EXECUTOR: AI — Wire badge into 3 surfaces (per `Q5`, badge only — exact paths per Pass 1 finding F2, applied 2026-05-06):
  - `_NotificationItemCard` in `lib/features/notifications/presentation/conversation_by_date_screen.dart` — read persona directly from the `NotificationItem` (per `Q1`). **Executed 2026-05-07**: 24px badge inline next to the priority chip (left-aligned Row).
  - `ConversationScreen` header in `lib/features/notifications/presentation/conversation_screen.dart` — read from bloc-cached `personasBySender` keyed on `senderId`. **Executed 2026-05-07**: AppBar title wrapped in `BlocSelector<NotificationBloc, NotificationState, VoicePersona?>` reading `state.personaFor(widget.senderId)` (state guarded by `is PersonaSnapshotMixin`); 28px badge + senderId text via Row when persona present.
  - Inbox sender tile in `lib/features/notifications/presentation/inbox_screen.dart` (CircleAvatar at `:141` per REUSE pre-pass) — same pattern as conversation header. **Executed 2026-05-07**: parent `InboxScreen.itemBuilder` passes `persona: state.personaFor(senderId)` to `_SenderTile`; tile renders `PersonaBadge` in `leading:` slot when non-null, falls back to existing CircleAvatar otherwise.
- [x] EXECUTOR: AI — Widget tests: persona present → badge renders with correct color (verified by finder + key); persona absent → no badge in tree; borrowed variant → dashed border verified. **Executed 2026-05-07**: 6 tests in `persona_badge_test.dart` (3.1 present+colored / 3.2 absent / 3.3 borrowed-dashed / 3.4a light+dark / 3.4b broken-emoji codepoint resilience / malformed-color defensive); +2 in `conversation_screen_test.dart` (3.5 header reads bloc-cached persona / header omits when empty).
- [x] EXECUTOR: AI — Run `./flutter.sh test test/widget/notifications/persona_badge_test.dart` — assert all 3 widget tests pass. **Executed 2026-05-07**: full baseline `./flutter.sh test test/unit/ test/widget/ test/service_integration/` returned `302 ✅ / 0 ❌` (was 294; +8). Phase 0 + Phase 2 regressions still green.
- [ ] EXECUTOR: HUMAN (subjective UX) — Final acceptance review: confirm badge color/contrast in light + dark mode meets visual taste. Per `00-working-contract.md` user-involvement gate item 3. **Pending: requires laptop+emulator deployment per `feedback_dev_server_laptop_split` memory rule.**
...
- [x] EXECUTOR: AI — **REUSE-AS-IS verified** at 2026-05-06: `StreamingTtsPlayer.speak()` already has the `String? voiceId` parameter (`streaming_tts_player.dart:130`) and the body wiring at `:141` already uses `if (voiceId != null) body['voice_id'] = voiceId`. Per REUSE pre-pass — see `00-index.md` "Prior art referenced". Phase 4 Task 4.1 collapses to **verification + comment**: add a code comment at `:130` flagging this parameter as the documented persona pipe-through path; confirm via test that body shape matches `Q3` spec. **Executed 2026-05-07**: 13-line dartdoc block added above `speak()` documenting the `voiceId` parameter as the persona pipe-through path, the absent-→-Sam server contract, and the deliberate `if (voiceId != null)` body-wiring choice; cross-referenced to `03-decisions.md` Q3.
- [x] EXECUTOR: AI — Add a code comment in the fallback path of `tts_orchestrator.dart` explicitly stating that `voiceId` is intentionally NOT piped through to the `flutter_tts` fallback (different voice space). Per `Q4`. The comment exists to prevent a future maintainer from "fixing" this; Pass 2 Adversarial would otherwise flag the missing test coverage that this comment forestalls. **Executed 2026-05-07**: 11-line comment block added inside `_speakViaFallback()` — explains ElevenLabs voice IDs vs `flutter_tts` device voices live in different namespaces; warns future maintainer against piping `voiceId` here.
- [x] EXECUTOR: AI — Wire orchestrator → `notification.voicePersona?.voiceId`. `tts_orchestrator.dart` reads the persona straight off the notification (per `Q1`) and passes through to the **already-wired** `streamingTtsPlayer.speak(text, voiceId: ...)`. **Executed 2026-05-07**: added `String? voiceId` param to `enqueueIfSpeakable`; added `voiceId` field to private `_Utterance` class; `_dispatchCurrent` now passes `voiceId: utter.voiceId` to `_player.speak`. NotificationBloc `_onExternalUpdate` updated to pass `voiceId: n.voicePersona?.voiceId` at the `enqueueIfSpeakable` call site.
- [x] EXECUTOR: AI — TTS unit tests covering existing-but-untested behavior + new orchestrator pipe-through: body includes `voice_id` when persona present; body omits `voice_id` when persona absent; `borrowed=true` does not change the call site; orchestrator calls `speak()` with `voiceId` from notification.voicePersona. **Executed 2026-05-07**: 6 new tests — 3 in `streaming_tts_player_test.dart` (4.1 voiceId in body / 4.2 voiceId omitted when null per Q3 absent-→-Sam contract / 4.3 borrowed body shape unchanged); 3 in `tts_orchestrator_test.dart` (4.4 persona piped from notification / 4.4b null voiceId defensive — verifies orchestrator passes `voiceId: null` cleanly through to player.speak / 4.5 quota fallback omits voiceId per Q4 + F11 — uses error-stream injection to enter the 5-min window then asserts `verify(fallback.flutterTtsSpeak(text))` and never `player.speak(voiceId: ...)`).
- [x] EXECUTOR: AI — Run `./flutter.sh test test/unit/services/tts/` — assert all new tests pass plus existing 10 regression tests stay green. **Executed 2026-05-07**: 27/27 in `test/unit/services/tts/` (was 21; +6); full baseline `test/unit/ test/widget/ test/service_integration/` returned `308 ✅ / 0 ❌` (was 302; +6). All Phase 0/1/2/3 regressions hold.
...
**Status**: ✅ AI portion COMPLETE 2026-05-07; HUMAN gate pending (laptop+emulator)

### Tasks

- [x] EXECUTOR: AI — Update `<mobile>/TODO.md` (per Pass 1 finding F4, applied 2026-05-06): (a) mark each of the 12 implementation tasks enumerated in §2-§6 of this doc as `[x]`; (b) add a single 'Completed (Recent)' entry referencing this milestone's commit hash; (c) leave all other open items unchanged. **Do NOT do a wider TODO sweep** — out of scope for this milestone. **Executed 2026-05-07**: TODO.md updated; Phases 0-4 already marked done across prior session entries; Phase 5 promoted to "Completed (Recent)" with milestone commit-hash slot reserved for the upcoming session-end commit.
- [x] EXECUTOR: AI — Update `<mobile>/history.md`: append a session-end entry per the standard template. **Executed 2026-05-07**: appended at session-end (the entry above this document).
- [x] EXECUTOR: AI — Run `./flutter.sh test` — assert full suite is `273 + N green / 0 failed` where N is the count of new tests added across phases 1-4 (~10). **Executed 2026-05-07**: full baseline `test/unit/ test/widget/ test/service_integration/` returned `308 ✅ / 0 ❌` (273 starting + 35 across Phases 0-4; the original "+10" estimate was for the original Phase 1-4 minimum, not actuals).
- [x] EXECUTOR: AI — Verify no quarantined tests reactivated (per `<global>~/.claude/CLAUDE.md` legacy quarantine rule). **Executed 2026-05-07**: `test/legacy_quarantine/` count is `0 +0 -44` — drift baseline unchanged from session `0d54c763`. No silent reactivations.
- [x] EXECUTOR: AI — Extend on-device TTS runbook with voice-persona section (added per user direction 2026-05-07 to bundle BOTH outstanding HUMAN gates). **Executed 2026-05-07**: 7 new acceptance steps added to `src/rnd/v0.1.7/2026.04.24-on-device-tts-verify-runbook.md` — vp1 inbox badge / vp2 conversation header badge / vp3 by-date item badge / vp4 borrowed dashed border / vp5 light+dark contrast (Phase 3 §3.6 HUMAN gate) / vp6 persona-voice timbre vs Sam (Phase 4 F5 HUMAN gate) / vp7 quota-fallback uses device flutter_tts (Q4 audible verification). Persona timbre cheat sheet for the 6 voices documented inline.
- [ ] EXECUTOR: HUMAN — laptop + Android SDK access required — Run existing TTS verify runbook at `<mobile>/src/rnd/v0.1.7/2026.04.24-on-device-tts-verify-runbook.md` scenarios 5-7 (ElevenLabs primary path; no new on-device step per `Q6`). **Additional assertion for this milestone** (per Pass 1 finding F5, applied 2026-05-06): confirm voice timbre matches session's allocated persona — e.g., if `get_session_info()` shows Adam allocated, voice heard should be deep male (not Sam's neutral default). Record the persona name + perceived voice character in the session's progress note. **Plus the bundled Phase 3 visual gate** (vp1-vp5) and Phase 4 quota-fallback audible gate (vp7).
...
| 3 — UI badge | ✅ complete | 2026-05-07 | 294 → 302 (+8) | uncommitted | New: `lib/shared/painters/dashed_border_painter.dart` (genuinely-new `CustomPainter` per Q9); `lib/features/notifications/presentation/persona_badge.dart` (StatelessWidget wrapping `CircleAvatar`, hex-color parsing with theme-primary fallback, F9 failure-mode contract — color-only is acceptable degradation); `test/widget/notifications/persona_badge_test.dart` (6 tests covering 3.1 present+colored / 3.2 absent / 3.3 borrowed dashed / 3.4a light+dark / 3.4b broken-emoji codepoint resilience / malformed-color defensive). Modified: `test_keys.dart` (+`personaBadgePrefix`, `personaBadgeDashedPrefix`); `inbox_screen.dart` (`_SenderTile` accepts `persona` from parent's `state.personaFor(senderId)`; PersonaBadge in leading slot when persona present, falls back to existing CircleAvatar); `conversation_screen.dart` (AppBar title now `BlocSelector` reading `personasBySender[widget.senderId]`; renders 28px badge + senderId text via Row when persona present, plain text when absent); `conversation_by_date_screen.dart` (`_NotificationItemCard` reads `item.voicePersona` directly per Q1; 24px badge inline next to priority chip); `conversation_screen_test.dart` (+2 new tests: 3.5 header reads bloc-cached persona / header omits badge when personasBySender empty). |
| 4 — TTS routing | ✅ complete | 2026-05-07 | 302 → 308 (+6) | uncommitted | Verify+comment phase per REUSE pre-pass. 13-line dartdoc on `streaming_tts_player.dart:speak()` documenting voiceId as persona pipe-through (Q3); 11-line "intentional omit" comment in `tts_orchestrator.dart:_speakViaFallback` (Q4). Wired `enqueueIfSpeakable.voiceId` → `_Utterance.voiceId` → `_player.speak(voiceId:)`. NotificationBloc `_onExternalUpdate` now passes `voiceId: n.voicePersona?.voiceId` at the `enqueueIfSpeakable` call site. 6 new tests (3 in streaming_tts_player_test.dart for 4.1/4.2/4.3; 3 in tts_orchestrator_test.dart for 4.4/4.4b/4.5). |
| 5 — docs + verify | ✅ AI complete; ⏳ HUMAN gate pending | 2026-05-07 | 308 ✅ / 0 ❌ baseline confirmed; 44 ❌ quarantine unchanged | uncommitted (this Phase-5 close batch) | All 5 AI tasks complete: TODO.md + history.md milestone-close entries; full baseline + quarantine re-run; runbook extended with 7 new persona acceptance steps (vp1-vp7) bundling BOTH outstanding HUMAN gates. HUMAN runbook execution is the only remaining step; gated on laptop+emulator deployment per `feedback_dev_server_laptop_split` memory rule. |
